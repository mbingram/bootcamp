...
export const loadTokens = async (provider, addresses, dispatch) => {
    ...
    token = new ethers.Contract(addresses[0], TOKEN_ABI, provider) // (address, abi, provider)
    symbol = await token.symbol()
    dispatch({ type: 'TOKEN_1_LOADED', token, symbol })

    token = new ethers.Contract(addresses[1], TOKEN_ABI, provider) // (address, abi, provider)
    symbol = await token.symbol()
...
export const loadAllOrders = async (provider, exchange, dispatch) => {
    ...
    // Fetch all orders
    const orderStream = await exchange.queryFilter('Order', 0, block) // contract.queryFilter( event, from block/hash, to block/hash)
    const allOrders = orderStream.map(event => event.args) // returns the arguments from the event itself
...
export const transferTokens = async (provider, exchange, transferType, token, amount, dispatch) => {
    ...
    // fetch signer from contract
    try {
        const signer = await provider.getSigner()
        // parse amount to string
        const amountToTransfer = ethers.utils.parseUnits(amount.toString(), 18)
        // check if transfer is deposit or withdrawal
...
export const cancelOrder = async (provider, exchange, order, dispatch) => {
    ...
    try {
        const signer = await provider.getSigner()
        const transaction = await exchange.connect(signer).cancelOrder(order.id) // call the cancelOrder function from the exchange contract
        await transaction.wait()
    } catch (error) {
